...
def step(action):
    ...
    #norm_image = cv2.normalize(obs, obs, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    #print(norm_image[0][0])
    #cv2.imshow("obs",obs )
    #cv2.waitKey(delay=1)
    slam_frame_count=slam_frame_count+0.5
    slam.process_image_mono(obs, slam_frame_count)
    if reward > 0:
